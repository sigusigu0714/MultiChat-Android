...
class CoreTest {
    ...
    @Test fun profilesRejectUnsafeEndpointsAndSecrets() {
        listOf("http://example.com","https://name:password@example.com","https://example.com?token=x","https://example.com#secret","https:///missing","https://example.com:99999","https://example.com:0").forEach { bad -> assertThrows(IllegalArgumentException::class.java) { Profile(serverURL=bad).validated() } }
    ...
    @Test fun speechExclusionsAndLength() { val s=Settings(ttsEnabled=true,ignoredUsers="bot Demo"); assertNull(speechText(event(),s)); val spoken=speechText(event(message="あいうえお".repeat(50)),s.copy(ignoredUsers=""))!!; assertTrue(spoken.endsWith("以下省略")); assertTrue(spoken.length<210) }
    @Test fun speechCompressesRepeatedCharacters() { assertEquals("Demo、あああ",speechText(event(message="あ".repeat(200)),Settings(ttsEnabled=true))) }
