...
## どねるの通知
チャットを連携していなくても、設定 →「どねる」にAlert BoxのHTTPS URLを入力できます。
「保存して再生」を押して設定を閉じ、MultiChat画面を表示したまま、どねる側の通知テストで映像と音を確認してください。
URLは端末内の安全な保存領域に保存します。寄付ページURLやOBSリレーURLとは異なります。
アプリのバックグラウンド・画面ロック中の通知再生には対応しません。
実際のどねるアカウントからの映像・音声は利用環境での確認が必要です。
